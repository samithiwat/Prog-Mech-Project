	...
	public static void invUpdate() {
		for(int i = 0 ; i < MapOverview.allFightOverlay.size() ; i++) {
			...
			for(int j = 0 ; j < overlay.challenger_weaponCard.size() ; j++) {
				...
				if(card.findCard(card.getKey()) == -1 || FightController.challenger_slot.size() >= challenger.getMyMinion().size()+1  ) {
...
//				System.out.println(card.findCard(card.getKey()));
				if(card.findCard(card.getKey()) == -1 || FightController.challenged_slot.size() >= challenged.getMyMinion().size()+1  ) {
					card.setVisible(false);
				}
				else {
					card.setVisible(true);
