...
public class GameLaw {
	...
	public void activateEachTurn(MainCharacter character) {
		...
		if (this.giftPerGoodPoint) {
			character.setMoney(character.getMoney() + character.getGoodPoint() * MainCharacter.M);
		}

		if (this.giftNoWeapon && character.getWeaponHand().size() == 0) {
			character.setMoney(character.getMoney() + 5 * MainCharacter.M);
		}
	...
	public void activateEachCycle() {
		if (this.setMoneyToAverage) {
			...
			GameSetUp.theGovernment
					.setMoney(averageSum + Math.max(0, sum - averageSum * GameSetUp.gameCharacter.size()));
		}
	}
